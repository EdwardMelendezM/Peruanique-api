# Skill: nextjs-sodu-interface-generator
...
2. **State Management**: Usar `useForm` con `zodResolver`.
3. **Smart Feedback (Sonner)**:
...
4. **Error Mapping**: Si el `result.success` es falso y existen `fieldErrors`, mapearlos al formulario usando `setError`.
5. **Loading UI**: Implementar el estado `isSubmitting` para deshabilitar el `Button` y mostrar el `Loader2`.
